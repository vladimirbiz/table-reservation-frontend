...
function GreenZone({ changeData, getData }) {
    const [seatStates, setSeatStates] = useState(Array(147).fill(null)); // Array to hold seat data

    useEffect(() => {
      let isMounted = true; // Track if component is mounted
      const fetchInitialData = async () => {
        const initialStates = await Promise.all(
          Array.from({ length: 9 }, (_, seatId) => getData(seatId))
        );
        if (isMounted) {
    ...
    const seats = Array.from({ length: 9 }, (_, i) => {
      const seatId = i+138; // Adjust the seat ID
      const isBlack = seatStates[seatId] === true; // Check if seat data is true
  
      return (
        <div key={seatId} className={`seat ${isBlack ? 'black' : 'green'}`}>
